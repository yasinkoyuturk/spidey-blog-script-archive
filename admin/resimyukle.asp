<%
If Session("blogadmin")=False Then Response.Redirect "default.asp"

Dim Fs, Kok, DosyaAdi, Yukle, YeniDosya, xI, xS

Set Yukle=New DosyaYukleme

Kok=Server.Mappath("\") & "\blog\upload\resimler"
DosyaAdi=Yukle.FileName("dosya")

Set Fs=Server.CreateObject("Scripting.FileSystemObject")

If Fs.FileExists(Kok & "\" & DosyaAdi)=True Then
		Response.redirect "default.asp?goster=resimler"
	Set Yukle=Nothing
	Set Fs=Nothing
Else
	Set YeniDosya=Fs.CreateTextFile(Kok & "\" & DosyaAdi)
	
	For xI=1 To LenB(Yukle.Value("dosya"))
	   YeniDosya.Write Chr(AscB(MidB(Yukle.Value("dosya"), xI, 1)))
	Next
